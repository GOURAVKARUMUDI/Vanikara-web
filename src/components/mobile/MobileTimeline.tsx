    ...
    <div className="relative py-12 px-2">
      ...
      <div className="space-y-8 relative">
        {milestones.map((item, index) => {
          ...
          return (
            <div key={item.title} className="relative pl-9 flex flex-col">
              {/* Timeline Indicator Ring Node */}
              <div className="absolute left-[11px] top-2.5 w-3.5 h-3.5 rounded-full bg-slate-900 border-2 border-[var(--accent-color)] z-10 shadow-sm" />

              {/* Expandable card block */}
              <div
                onClick={() => toggleExpand(index)}
                className={`p-5 rounded-2xl bg-[var(--glass-bg)] border transition-all duration-300 shadow-sm cursor-pointer select-none ${
